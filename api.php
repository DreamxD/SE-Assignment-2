...
/**
 * RongCloud server API interface  1.0
 * Class ServerAPI
 * @author  caolong
...
class ServerAPI{
    ...
    private $format;                //data format json/xml
    ...
    public function getToken($userId, $name, $portraitUri) {
        try{
            if(empty($userId))
                throw new Exception('userID cannot be blank');
        //    if(empty($name))
        //        throw new Exception('username cannot be blank');
        //    if(empty($portraitUri))
        //        throw new Exception('user avatars cannot be blank');
    ...
    /**
     * send message
     * @param $fromUserId   message sender's Id。（requisite）
     * @param $toUserId     message receiver's Id。（requisite）
     * @param $objectName   message type refer to RongCloud message Type list（requisite）
     * @param $content      发送消息内容，参考融云消息类型表.示例说明；如果 objectName 为自定义消息类型，该参数可自定义格式。（requisite）
     * @param string $pushContent   如果为自定义消息，定义显示的 Push 内容。(requisite)
     * @param string $pushData  针对 iOS 平台，Push 通知附加的 payload 字段，字段名为 appData。(requisite)
     * @return json|xml
     */
    public function messagePublish($fromUserId, $toUserId = array(), $objectName, $content, $pushContent='', $pushData = '') {
        try{
            if(empty($fromUserId))
                throw new Exception('message senderid cannot be blank');
            if(empty($toUserId))
                throw new Exception('message receiver Id cannot be blank');
            if(empty($objectName))
                throw new Exception('message type cannot be blank');
            if(empty($content))
     ...
     * Group message
     * @param $fromUserId           message sender's Id。（requisite）
     * @param $toGroupId             接收群Id，提供多个本参数可以实现向多群发送消息。（requisite）
     * @param $objectName           消息类型，参考融云消息类型表.消息标志；可自定义消息类型。（requisite）
     * @param $content              发送消息内容，参考融云消息类型表.示例说明；如果 objectName 为自定义消息类型，该参数可自定义格式。（requisite）
     * @param string $pushContent   如果为自定义消息，定义显示的 Push 内容。(requisite)
     * @param string $pushData      针对 iOS 平台，Push 通知附加的 payload 字段，字段名为 appData。(requisite)
     * @return json|xml
